...
The configuration involves leaking SSH hosts and paths, so first, we need to decrypt it:
#+begin_src bash
gpg -d -o mutagen.yml mutagen.yml.asc
#+end_src
Updates imply a manual synchronization:
#+begin_src bash
# Encrypt
gpg -e -a -r 9CCCE36402CB49A6 -o mutagen.yml.asc mutagen.yml
#+end_src
*** Usage
