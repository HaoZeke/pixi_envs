...
def _detect_cpu_simd(
    ...
    """Microbench algorithm=auto against algorithm=cell_list on a small system.

    Returns True if auto is at least (1 - threshold_ratio) faster than
    cell_list, i.e. cluster-pair is present and routes through a different
    code path. Returns False when the two times are within noise (the
    build does not have cluster-pair, so the auto-variant series would
    duplicate the cell_list series and pollute the plot).
    """
    from vesin import NeighborList

    positions, box = generate_system(probe_atoms, density, seed)

    def _min_ms(algorithm: str) -> float:
        nl = NeighborList(
            cutoff=cutoff, full_list=True, sorted=False, skin=0.0, algorithm=algorithm
        )
        # one warmup call (NVRTC compile / cache prime is not relevant here, but
        # the very first call always pays a small dispatch cost)
        nl.compute(points=positions, box=box, periodic=True, quantities="ij", copy=False)
        best = float("inf")
    ...
    cell = _min_ms("cell_list")
    auto = _min_ms("auto")
    enable = auto < cell * threshold_ratio
    print(
        f"cpu_simd auto-detect: cell_list={cell:.3f} ms, auto={auto:.3f} ms "
        f"(ratio {auto / cell:.2f}); include_cpu_simd={'yes' if enable else 'no'}",
        flush=True,
    )
    return enable
...
def run_benchmark(args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    if args.include_cpu_simd is None:
        args.include_cpu_simd = _detect_cpu_simd(args.cutoff, args.density, args.seed)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    run.add_argument(
        ...
        default=None,
        help=(
            "measure CPU auto-dispatch as a SIMD cluster-pair variant. "
            "Default (None) runs a quick microbench against algorithm=cell_list "
            "at N=512 and enables the variant only when algorithm=auto is at "
            "least 20%% faster (i.e. the build actually has cluster-pair). "
            "Pass --include-cpu-simd to force on, --no-include-cpu-simd to "
            "force off."
        ),
    )
