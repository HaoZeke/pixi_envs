...
Configuration and benchmark code live in ~asv.conf.json~ and ~benchmarks/~ (in
the pixi_envs directory, not inside the gromacs submodule), following the [[https://github.com/TheochemUI/eOn][eOn]] layout.
...
# Run from pixi_envs/orgs/metatensor/gromacs/
...
GMX_BIN=$(realpath .pixi/envs/metatomic-cpu/bin/gmx) \
  ...
  'asv run -E "existing:$(which python)" --set-commit-hash $(git -C gromacs rev-parse HEAD) --record-samples --quick'
#+end_src

*** Comparing two commits
#+begin_src bash
pixi run -e metatomic-cpu bash -c 'asv compare <hash1> <hash2>'
#+end_src

*** CI
PRs to the GROMACS fork trigger two workflows:
- ~ci_benchmark.yml~: builds base + PR, runs ASV on both, uploads results
- ~ci_bench_commenter.yml~: posts comparison table as a PR comment (via [[https://github.com/HaoZeke/asv-perch][asv-perch]])
