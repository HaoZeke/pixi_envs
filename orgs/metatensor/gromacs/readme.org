...
#+begin_src bash
# local compilers
bless -- cmake ..     -DGMX_BUILD_OWN_FFTW=OFF     -DCMAKE_CXX_COMPILER_LAUNCHER=sccache     -DCMAKE_C_COMPILER_LAUNCHER=sccache     -DGMX_MPI=OFF -DGMX_METATOMIC=yes     -DBUILD_TESTING=OFF     -DCMAKE_PREFIX_PATH="$CMAKE_PREFIX_PATH"     -DCMAKE_BUILD_TYPE=Debug     -DDOWNLOAD_VESIN=yes     -DGPU_DEVICE=CUDA  -DBUILD_TESTING=ON
